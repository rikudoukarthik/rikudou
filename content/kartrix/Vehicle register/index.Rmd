...
i_am("content/kartrix/Vehicle register/index.Rmd")

cur_path <- "content/kartrix/Vehicle register/"
...
table_caption <- glue("Leaderboard of own vehicles I have driven, based on total distance driven. Ranking priority is higher for logged distance than for total, owing to the uncertainty associated with approximation.")
...
```{r, dev.args=list(bg = "transparent"), include=TRUE, warning=FALSE}
data_veh %>% 
  filter(DRIVER == "Self") %>% 
...
**Last odometer reading**: `r data_veh %>% filter(VEHICLE == "Gixxer Rogue", !is.na(ODO)) %>% reframe(ODO = max(ODO)) %>% pull(ODO)` km
